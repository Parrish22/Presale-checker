...
# Initialize Cloudscraper session (bypasses Cloudflare anti-bot protection)
...
def send_pinksale_discord_alert(presale_data):
    ...
    embed = {
        ...
        "footer": {"text": "Verified PinkSale.com Direct Route • Hard Cap Method"},
...
def fetch_pinksale_hardcap_presales():
    """Queries PinkSale active pool directory using Cloudscraper to bypass Cloudflare."""
    ...
    for url in endpoints:
        try:
            # Use cloudscraper instead of standard requests
            res = scraper.get(url, timeout=15)
            print(f"  └─ Request {url} returned HTTP {res.status_code}")
...
if __name__ == "__main__":
    print("PinkSale Screener Active (Cloudflare Challenge Bypass Enforced)...")
    while True:
        try:
            run_screener()
        except Exception as e:
