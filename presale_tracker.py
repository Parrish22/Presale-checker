...
# Browser Emulation Headers to bypass Cloudflare Protection
PINKSALE_HEADERS = {
...
def build_pinksale_url(pool_id, chain):
    """Guarantees 100% valid PinkSale Launchpad URL."""
    c = str(chain).upper()
...
def is_hardcap_active(pool_data):
    ...
    1. Rejects presales where status is 'filled', 'completed', or 'ended'.
    2. Rejects presales where total raised reaches 100% of Hard Cap.
    """
    status = str(pool_data.get("status", "")).lower()
    if status in ["filled", "completed", "ended", "cancelled", "success"]:
        return False

    # Check hardcap numbers if exposed
    ...
    if hard_cap > 0 and raised >= hard_cap:
        return False  # Hard cap hit -> presale is over
...
def send_pinksale_discord_alert(presale_data):
    ...
    embed = {
        ...
        "url": pinksale_url,  # Clicking title opens PinkSale directly
        ...
        "footer": {"text": "Verified PinkSale.com Direct Route • Hard Cap Incomplete"},
...
def fetch_pinksale_hardcap_presales():
    """Queries PinkSale active pool directory across SOL, ETH, and BSC."""
    print("🔍 Fetching Active Hard Cap Presales from PinkSale...")

    target_chains = ["bsc", "ethereum", "solana", "sol", "eth", "bnb"]
    
    # PinkSale V1/V2 Public Listing Endpoints
    endpoints = [
    ...
    for url in endpoints:
        try:
            res = requests.get(url, headers=PINKSALE_HEADERS, timeout=12)
            if res.status_code != 200:
                print(f"  └─ HTTP {res.status_code} received from {url}. (Cloudflare bypass active)")
                continue
            ...
            for pool in pools:
                ...
                # 2. FILTER: MUST BE UNCOMPLETED HARD CAP
                if not is_hardcap_active(presale_payload):
                    print(f"  └─ Skipped {pool_id}: Hard cap reached or status ended.")
                    continue
    ...
    print(f"--- Completed Scan. Processed {found_count} new PinkSale hard cap presales. ---")
...
if __name__ == "__main__":
    print("PinkSale Screener Service Active (Hard Cap Method Enforced)...")
    while True:
        try:
            run_screener()
        except Exception as e:
            print(f"Loop Exception: {e}")
        time.sleep(180)  # Scan PinkSale every 3 minutes
